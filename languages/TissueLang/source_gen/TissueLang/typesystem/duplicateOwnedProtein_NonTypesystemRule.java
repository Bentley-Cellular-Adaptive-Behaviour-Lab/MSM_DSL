...
public class duplicateOwnedProtein_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode cell_type, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    for (SNode current_reference : ListSequence.fromList(SLinkOperations.getChildren(cell_type, LINKS.Owned_Species$sIXq))) {
      String current_name = SPropertyOperations.getString(SLinkOperations.getTarget(current_reference, LINKS.target$26Ka), PROPS.name$MnvL);
      for (SNode query_reference : ListSequence.fromList(SLinkOperations.getChildren(cell_type, LINKS.Owned_Species$sIXq))) {
        String query_name = SPropertyOperations.getString(SLinkOperations.getTarget(query_reference, LINKS.target$26Ka), PROPS.name$MnvL);
        if (Objects.equals(query_name, current_name) && !(Objects.equals(query_reference, current_reference))) {
          {
            final MessageTarget errorTarget = new NodeMessageTarget();
            IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(cell_type, "Duplicate owned species are not allowed: " + current_name, "r:583b5986-f053-4561-9682-b6e844b2b168(TissueLang.typesystem)", "1783685413826403788", null, errorTarget);
          }
        }
      }
    }
