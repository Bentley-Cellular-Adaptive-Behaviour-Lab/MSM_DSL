...
public class check_StartingConc_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode species, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    if ((float) Species__BehaviorDescriptor.getStartConcentrationValue_id3fk35jmCFN3.invoke(species) < 0) {
      {
        ...
        IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(SLinkOperations.getTarget(species, LINKS.Starting_Concentration$a3uk), "Starting concentration cannot be less than zero.", "r:9e0fbf2a-b9f3-458b-86a2-82f541ac8497(SpeciesLang.typesystem)", "2231199800374304111", null, errorTarget);
      }
    }
    if ((float) Species__BehaviorDescriptor.getStartConcentrationValue_id3fk35jmCFN3.invoke(species) < (float) Species__BehaviorDescriptor.getMinConcentrationValue_id1VQO6m$d9Os.invoke(species)) {
      {
        final MessageTarget errorTarget = new NodeMessageTarget();
        IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(SLinkOperations.getTarget(species, LINKS.Starting_Concentration$a3uk), "Starting concentration cannot be less than minimum concentration.", "r:9e0fbf2a-b9f3-458b-86a2-82f541ac8497(SpeciesLang.typesystem)", "2231199800374313909", null, errorTarget);
      }
    }
    if ((float) Species__BehaviorDescriptor.getStartConcentrationValue_id3fk35jmCFN3.invoke(species) > (float) Species__BehaviorDescriptor.getMaxConcentrationValue_id1VQO6m$daTe.invoke(species)) {
      {
        final MessageTarget errorTarget = new NodeMessageTarget();
        IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(SLinkOperations.getTarget(species, LINKS.Starting_Concentration$a3uk), "Starting concentration cannot be greater than maximum concentration.", "r:9e0fbf2a-b9f3-458b-86a2-82f541ac8497(SpeciesLang.typesystem)", "2231199800374315212", null, errorTarget);
      }
    }
  }
