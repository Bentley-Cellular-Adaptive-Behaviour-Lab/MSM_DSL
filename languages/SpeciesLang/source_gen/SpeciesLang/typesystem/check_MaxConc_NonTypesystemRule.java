...
public class check_MaxConc_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode species, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    if ((float) Species__BehaviorDescriptor.getMaxConcentrationValue_id1VQO6m$daTe.invoke(species) < 0) {
      {
        ...
        IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(SLinkOperations.getTarget(species, LINKS.Max_Concentration$S2Fq), "Minimum concentration cannot be less than zero.", "r:9e0fbf2a-b9f3-458b-86a2-82f541ac8497(SpeciesLang.typesystem)", "1252520794726879416", null, errorTarget);
      }
    }
    if ((float) Species__BehaviorDescriptor.getMaxConcentrationValue_id1VQO6m$daTe.invoke(species) < (float) Species__BehaviorDescriptor.getMinConcentrationValue_id1VQO6m$d9Os.invoke(species)) {
      {
        final MessageTarget errorTarget = new NodeMessageTarget();
        IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(SLinkOperations.getTarget(species, LINKS.Max_Concentration$S2Fq), "Maximum concentration cannot be less than maximum concentration.", "r:9e0fbf2a-b9f3-458b-86a2-82f541ac8497(SpeciesLang.typesystem)", "1252520794726879428", null, errorTarget);
      }
    }
  }
