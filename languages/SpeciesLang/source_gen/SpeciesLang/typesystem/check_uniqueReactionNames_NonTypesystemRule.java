        ...
        if (Objects.equals(SPropertyOperations.getString(queryProcess, PROPS.name$MnvL), SPropertyOperations.getString(Process, PROPS.name$MnvL)) && !(Objects.equals(Process, queryProcess))) {
          {
            ...
            IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(Process, "Process names must be unique.", "r:9e0fbf2a-b9f3-458b-86a2-82f541ac8497(SpeciesLang.typesystem)", "8050503989175222449", null, errorTarget);
          }
        }
      }
      for (SNode currentSpecies : ListSequence.fromList(SLinkOperations.getChildren(container, LINKS.Species$hnnc))) {
        if (Objects.equals(SPropertyOperations.getString(currentSpecies, PROPS.name$MnvL), SPropertyOperations.getString(Process, PROPS.name$MnvL))) {
          {
            final MessageTarget errorTarget = new NodeMessageTarget();
            IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(Process, "Process names must be unique.", "r:9e0fbf2a-b9f3-458b-86a2-82f541ac8497(SpeciesLang.typesystem)", "8015826073199965026", null, errorTarget);
          }
        }
      }
      for (SNode currentParameter : ListSequence.fromList(SLinkOperations.getChildren(container, LINKS.Parameters$hoyh))) {
        if (Objects.equals(SPropertyOperations.getString(currentParameter, PROPS.name$MnvL), SPropertyOperations.getString(Process, PROPS.name$MnvL))) {
          {
            final MessageTarget errorTarget = new NodeMessageTarget();
            IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(Process, "Process names must be unique.", "r:9e0fbf2a-b9f3-458b-86a2-82f541ac8497(SpeciesLang.typesystem)", "8015826073199970672", null, errorTarget);
          }
        }
      }
    }
