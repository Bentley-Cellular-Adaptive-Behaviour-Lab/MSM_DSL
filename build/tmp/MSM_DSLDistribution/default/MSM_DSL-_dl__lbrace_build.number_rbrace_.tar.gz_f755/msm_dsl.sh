#!/bin/sh
# ---------------------------------------------------------------------
# MSM_DSL startup script.
# ---------------------------------------------------------------------
# Generated by MPS

message()
{
  TITLE="Cannot start MSM_DSL"
...
# Try (in order): $MSM_DSL_JDK, .../msm_dsl.jdk, .../jbr, .../jbr-x86, $JDK_HOME, $JAVA_HOME, "java" in $PATH.
# ---------------------------------------------------------------------
IDEA_VENDOR_NAME='null'
# shellcheck disable=SC2154
if [ -n "$MSM_DSL_JDK" ] && [ -x "$MSM_DSL_JDK/bin/java" ]; then
  JDK="$MSM_DSL_JDK"
fi

if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/MSM_DSL0.1/msm_dsl.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/MSM_DSL0.1/msm_dsl.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  ...
  message "No JDK found. Please validate either MSM_DSL_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  fi
  exit 1
fi
...
# ---------------------------------------------------------------------
# shellcheck disable=SC2154
if [ -n "$MSM_DSL_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$MSM_DSL_PROPERTIES"
fi
...
elif [ -r "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/MSM_DSL0.1/msm_dsl$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/MSM_DSL0.1/msm_dsl$BITS.vmoptions"
elif [ -r "$IDE_BIN_HOME/msm_dsl$BITS.vmoptions" ]; then
  # default, standard installation
  VM_OPTIONS_FILE="$IDE_BIN_HOME/msm_dsl$BITS.vmoptions"
else
  # default, universal package
  test "$OS_TYPE" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
  VM_OPTIONS_FILE="$IDE_BIN_HOME/$OS_SPECIFIC/msm_dsl$BITS.vmoptions"
fi
...
# shellcheck disable=SC2154
if [ -n "$MSM_DSL_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$MSM_DSL_CLASSPATH"
fi
...
IDEA_PATHS_SELECTOR=MSM_DSL0.1
"$JAVA_BIN" \
  ${AGENT} \
  -classpath "$CLASSPATH" \
  ${VM_OPTIONS} \
  "-XX:ErrorFile=$HOME/java_error_in_MSM_DSL_%p.log" \
  "-XX:HeapDumpPath=$HOME/java_error_in_MSM_DSL.hprof" \
