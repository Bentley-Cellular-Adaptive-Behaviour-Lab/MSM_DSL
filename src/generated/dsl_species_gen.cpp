...
void ODEs::check_cell_only_ODEs(EC *ec) {
 ...
 // Parameter Definitions
 	double VEGFR_INHIB = calc_VEGFR_INHIB_rate(VEGFR, NOTCH);
 	...
 	double DLL4_UPREG = calc_DLL4_UPREG_rate(VEGF_VEGFR, DELTA);
 	double NOTCH_PROD = calc_NOTCH_PROD_rate(NOTCH_DIFF);
 // ODE Definitions
 	dxdt[0] = -(DLL4_NOTCH_DEG);
 	dxdt[1] = +(VEGFR_PROD)-(VEGFR_DEG)-(VEGFR_INHIB);
 	dxdt[2] = -(VEGF_VEGFR_DEG);
 	dxdt[3] = +(DLL4_DIFF)-(DLL4_DEG)+(DLL4_UPREG);
 	dxdt[4] = 0;
...
void ODEs::Endothelial_run_memAgent_ODEs(MemAgent* memAgent) {
  ...
  // Parameter Definitions
  	...
  	double NOTCH_PROD = calc_NOTCH_PROD_rate(NOTCH_DIFF);
  // ODE Definitions
  	dxdt[0] = -(DLL4_NOTCH_DEG)+(DLL4_NOTCH_ON)*1;
  	...
  	dxdt[3] = +(DLL4_DIFF)-(DLL4_DEG)-(DLL4_NOTCH_ON)*1+(DLL4_UPREG);
  	dxdt[4] = -(VEGF_VEGFR_ON)*1+(VEGF_VEGFR_OFF)*1;
  	dxdt[5] = +(NOTCH_PROD)-(NOTCH_DEG)-(DLL4_NOTCH_ON)*1;
  	dxdt[6] = 0;
  	dxdt[7] = 0;
  }
  ...
  void ODEs::Endothelial_run_cell_only_ODEs(EC *ec) {
  	...
  	ec->set_cell_protein_level("VEGF_VEGFR", states[2], 28);
