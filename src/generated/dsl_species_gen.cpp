...
void ODEs::EndothelialType_cell_system(const EndothelialType_cell_ode_states &x, EndothelialType_cell_ode_states &dxdt,
    ...
    double NOTCH_Diff = calc_NOTCH_Diff_rate(NOTCH, DLL4, adjacent_NOTCH); // FINE.
    ...
    dxdt[1] = +(beta) - (Hey_Degradation) + (HEY_Reg); // FINE.
    dxdt[2] = +(VR_Production) - (VR_Degradation) - (k1) * 1 + (k_1) * 1 - (k3); // FINE.
    dxdt[3] = -(V_VR_Degradation) + (k1) * 1 - (k_1) * 1; // FINE.
    dxdt[4] = +(DLL4_Diff) - (D_Degradation) - (k2) * 1 + (k_2) * 1 + (Dll4_Reg); // FINE.
    dxdt[5] = -(D_N_Degradation) + (k2) * 1 - (k_2) * 1; // FINE.
    dxdt[6] = -(I_Degradation) + (k4); // FINE.
    dxdt[7] = +(N_Production) - (N_Degradation) - (k2) * 1 + (k_2) * 1; // FINE.
    dxdt[8] = 0; // FINE.
    dxdt[9] = 0; // FINE.
}
