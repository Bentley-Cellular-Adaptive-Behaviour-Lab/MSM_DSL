...
void ODEs::check_cell_ODEs(EC *ec) {
	...
	if (ec->m_cell_type->m_name == "HUVEC") {
		HUVEC_run_cell_ODEs(ec);
	}
}

void ODEs::check_memAgent_ODEs(const std::string& cell_type_name, MemAgent *memAgent) {
	assert(this->get_ODE_TYPE() != -1);
 	if (cell_type_name == "HUVEC") {
 		HUVEC_run_memAgent_ODEs(memAgent);
 	}
}
...
void ODEs::check_cell_only_ODEs(EC *ec) {
	assert(this->get_ODE_TYPE() != -1);
	if (ec->m_cell_type->m_name == "HUVEC") {
		HUVEC_run_cell_only_ODEs(ec);
	}
}


 void ODEs::HUVEC_cell_system(const HUVEC_cell_ode_states &x, HUVEC_cell_ode_states &dxdt, double t) {
 // Species Definitions
 ...
 void ODEs::HUVEC_run_cell_ODEs(EC *ec) {
 	HUVEC_cell_ode_states states;
 	typedef odeint::runge_kutta_cash_karp54<HUVEC_cell_ode_states> error_stepper_type;
 	...
 	integrate_adaptive(controlled_stepper, HUVEC_cell_system, states, 0.0, 1.0, 0.1);
...
void ODEs::HUVEC_memAgent_system(const HUVEC_memAgent_ode_states &x, HUVEC_memAgent_ode_states &dxdt, double t) {
// Species Definitions
	double VEGF_VEGFR = x[0];
	double DLL4 = x[1];
	double VEGFR = x[2];
...
void ODEs::HUVEC_run_memAgent_ODEs(MemAgent* memAgent) {
	HUVEC_memAgent_ode_states states;
	typedef odeint::runge_kutta_cash_karp54<HUVEC_cell_ode_states> error_stepper_type;
	...
	typedef odeint::controlled_runge_kutta< error_stepper_type > controlled_stepper_type;
	controlled_stepper_type controlled_stepper;
	integrate_adaptive(controlled_stepper, HUVEC_memAgent_system, states, 0.0, 1.0, 0.1);
  ...
  void ODEs::HUVEC_cell_only_system(const HUVEC_cell_ode_states &x, HUVEC_cell_ode_states &dxdt, double t) {
  // Species Definitions
  	double DLL4 = x[0];
  ...
  void ODEs::HUVEC_run_cell_only_ODEs(EC *ec) {
  	HUVEC_cell_ode_states states;
  	typedef odeint::runge_kutta_cash_karp54<HUVEC_cell_ode_states> error_stepper_type;
  	...
  	states[1] = ec->get_cell_protein_level("DLL4_NOTCH", 0);
  	states[2] = ec->get_cell_protein_level("NOTCH", 0);
  	...
  	typedef odeint::controlled_runge_kutta< error_stepper_type > controlled_stepper_type;
  	controlled_stepper_type controlled_stepper;
  	integrate_adaptive(controlled_stepper, HUVEC_cell_only_system, states, 0.0, 1.0, 0.1);
