...
void ODEs::EndothelialType_cell_only_system(const EndothelialType_cell_only_ode_states &x, EndothelialType_cell_only_ode_states &dxdt, double t) {
	...
	double SEMA3A_PLEXIND1_9 = x[6];
	double DLL4_MEAN_9 = x[7];
	double NOTCH_MEAN_9 = x[8];
	double VEGF_VEGFR_0 = x[9];
	double SEMA3A_PLEXIND1_0 = x[10];
	double DLL4_NOTCH_0 = x[11];
	...
	dxdt[6] = 0; // SEMA3A_PLEXIND1_1
	dxdt[7] = 0; // DLL4_MEAN_1
	dxdt[8] = 0; // NOTCH_MEAN_1
	dxdt[9] = 0; // VEGF_VEGFR_10
	dxdt[10] = 0; // SEMA3A_PLEXIND1_10
	dxdt[11] = 0; // DLL4_NOTCH_10
}
...
void ODEs::EndothelialType_run_cell_only_ODEs(EC *ec) {
	...
	states[6] = ec->get_cell_protein_level("SEMA3A_PLEXIND1", 9);  //SEMA3A_PLEXIND1_1
	states[9] = ec->get_cell_protein_level("VEGF_VEGFR", 0);  //VEGF_VEGFR_10
	states[10] = ec->get_cell_protein_level("SEMA3A_PLEXIND1", 0);  //SEMA3A_PLEXIND1_10
	states[11] = ec->get_cell_protein_level("DLL4_NOTCH", 0);  //DLL4_NOTCH_10
	states[3] = ec->get_env_protein_level("VEGF") / agents;
	states[7] = ec->calc_adjacent_species_level("DLL4", false, true, 9);
	states[8] = ec->calc_adjacent_species_level("NOTCH", false, true, 9);
	...
	ec->set_cell_protein_level("SEMA3A_PLEXIND1", states[6], 10);
}
