...
void ODEs::Endothelial_cell_only_system(const Endothelial_cell_ode_states &x, Endothelial_cell_ode_states &dxdt, double t) {
    ...
    // ODE Definitions
    dxdt[0] = -(DLL4_NOTCH_DEG)+(DLL4_NOTCH_ON)*1-(DLL4_NOTCH_OFF)*1; // DLL4_NOTCH
    dxdt[1] = +(VEGFR_PROD)-(VEGFR_DEG)-(VEGF_VEGFR_ON)*1+(VEGF_VEGFR_OFF)*1-(VEGFR_INHIB); // VEGFR
    dxdt[2] = -(VEGF_VEGFR_DEG)+(VEGF_VEGFR_ON)*1-(VEGF_VEGFR_OFF)*1; // VEGF_VEGFR
    dxdt[3] = -(DLL4_DEG)-(DLL4_NOTCH_ON)*1+(DLL4_NOTCH_OFF)*1+(DLL4_UPREG)+(DLL4_DIFF)-(DLL4_INHIB); // DLL4
    dxdt[4] = -(VEGF_VEGFR_ON)*1+(VEGF_VEGFR_OFF)*1; // VEGF
//    dxdt[5] = +(NOTCH_PROD)-(NOTCH_DEG)-(DLL4_NOTCH_ON)*1+(DLL4_NOTCH_OFF)*1+(NOTCH_DIFF);
	dxdt[5] = 0; // Changed Notch dxdt to 0. // NOTCH
    dxdt[6] = -(SEMA_PLEXIN_ON)*1+(SEMA_PLEXIN_OFF)*1; // SEMA3E
    dxdt[7] = +(PLEXIND1_PROD)-(PLEXIND1_DEG)-(SEMA_PLEXIN_ON)*1+(SEMA_PLEXIN_OFF)*1; // PLEXIND1
    dxdt[8] = -(SEMA_PLEXIN_DEG)+(SEMA_PLEXIN_ON)*1-(SEMA_PLEXIN_OFF)*1; // SEMA_PLEXIN
    dxdt[9] = 0;
    dxdt[10] = 0;
}
...
static double calc_NOTCH_adjacent_level(EC *ec, bool memAgentODEs) {
    ...
    if (memAgentODEs) {
        ...
        level += ec->get_cell_protein_level("NOTCH", 0);
    }
    if (level == 0.0) {
        return 0.0;
    } else {
...
static double calc_DLL4_NOTCH_adjacent_level(EC *ec, bool memAgentODEs) {
    ...
    if (memAgentODEs) {
        ...
        level += ec->get_cell_protein_level("DLL4_NOTCH", 0);
    }
    if (level == 0.0) {
        return 0.0;
    } else {
...
static double calc_DLL4_adjacent_level(EC *ec, bool memAgentODEs) {
    ...
    if (memAgentODEs) {
        ...
        level += ec->get_cell_protein_level("DLL4", 0);
    }
    if (level == 0.0) {
        return 0.0;
    } else {
...
static double calc_DLL4_INHIB_rate(double DLL4, double SEMA_PLEXIN, double Nu) {
    Nu = 3;
    return 0.5*DLL4*pow(SEMA_PLEXIN,Nu); // Changed to 0.5
}