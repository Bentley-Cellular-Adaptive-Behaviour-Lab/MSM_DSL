...
void ODEs::check_cell_ODEs(EC *ec) {
  ...
  // ODE Definitions
  	...
  	dxdt[2] = -(D_N_Degradation)+(k2)*1-(k_2)*1;
  	dxdt[3] = -(I_Degradation)+(k4);
  	dxdt[4] = +(beta)-(FilopodiaTurnover)+(k5_FilProduction);
  	dxdt[5] = -(k1)*1+(k_1)*1+(k6_VEGFSensing);
  	dxdt[6] = +(beta)-(HEY_Degradation)+(HEY_Reg);
  	dxdt[7] = +(VR_Production)-(VR_Degradation)-(k1)*1+(k_1)*1-(k3);
  ...
  void ODEs::Endothelial_run_cell_ODEs(EC *ec) {
  	...
  	current_states[2] = ec->get_cell_protein_level("DLL4_NOTCH", 0);
  	current_states[3] = ec->get_cell_protein_level("NICD", 0);
  	current_states[4] = ec->get_cell_protein_level("FILOPODIA", 0);
  	current_states[5] = ec->get_cell_protein_level("VEGF", 0);
  	current_states[6] = ec->get_cell_protein_level("HEY", 0);
  	current_states[7] = ec->get_cell_protein_level("VEGFR", 0);
  	...
  	ec->set_cell_protein_level("DLL4_NOTCH", new_states[2], 1);
  	ec->set_cell_protein_level("NICD", new_states[3], 1);
  	ec->set_cell_protein_level("FILOPODIA", new_states[4], 1);
  	ec->set_cell_protein_level("VEGF", new_states[5], 1);
  	ec->set_cell_protein_level("HEY", new_states[6], 1);
  	ec->set_cell_protein_level("VEGFR", new_states[7], 1);
