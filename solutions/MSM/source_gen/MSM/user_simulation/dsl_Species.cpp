...
// Species Definitions //
	...
	double B = x[1];
// Rate Definitions //
	double rate_K1 = calc_K1_rate(A, B);
...
// ODE Definitions //
	dxdt[0] = -rate_K1*2 +rate_K2*2 -rate_A_deg +rate_A_prod; // Rate of change for species A
	dxdt[1] = -rate_K1*1 +rate_K2*1 -rate_B_deg +rate_B_prod; // Rate of change for species B
}

static double calc_K1_rate(double A, double B) {
	return 2.0 * A * B;
}
